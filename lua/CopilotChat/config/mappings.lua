...
return {
  ...
  accept_diff = {
    ...
    callback = function(source)
      ...
      local path = diff.parse_unified_diff(block.content)
      local bufnr = prepare_diff_buffer(path, source)
      local lines = vim.api.nvim_buf_get_lines(bufnr, 0, -1, false)
      local new_lines, applied = diff.apply_unified_diff(block.content, lines)
      if not applied then
        new_lines = utils.split_lines(block.content)
      end

      vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, new_lines)
      local first, last = diff.get_unified_diff_region(block.content, lines)
  ...
  show_diff = {
    ...
    callback = function(source)
      ...
      local new_lines, applied = diff.apply_unified_diff(block.content, original_lines)
      if not applied then
        new_lines = utils.split_lines(block.content)
      end
