...
local query = vim.treesitter.query.parse(
  ...
  (section
    (atx_heading
      (atx_h1_marker)
      heading_content: (_) @section_header
    )
    (_) @section_content
  )
  (section
    (fenced_code_block
      (info_string) @block_header
...
function Chat:parse()
  ...
  vim.print('Parsing chat buffer...')

  for id, node in query:iter_captures(root, self.bufnr, 0, -1) do
    local name = query.captures[id]
    local start_row, _, end_row, _ = node:range()
    vim.print(name .. ': ' .. node:type() .. ' (' .. start_row .. '-' .. end_row .. ')')
    ...
    elseif name == 'section_content' then
      ...
      current_message.content = current_message.content .. content
    elseif current_message.role == constants.ROLE.ASSISTANT then
      ...
      elseif name == 'block_content' then
        ...
        current_block.content = current_block.content .. content
      end
    end
  end
...
function Chat:render()
  ...
  for i, message in ipairs(self.messages) do
    ...
    if self.config.highlight_headers then
      ...
      vim.api.nvim_buf_set_extmark(self.bufnr, highlight_ns, message.section.start_line - 1, #header_value, {
        virt_text = {
    ...
    -- Highlight code block headers and show file info as virt lines
    for _, block in ipairs(message.section.blocks) do
      ...
      local start_line = header.start_line
      local end_line = header.end_line
      local text = string.format('[%s] %s', filetype, filename)
      if start_line and end_line then
        text = text .. string.format(' lines %d-%d', header.start_line, end_line)
      end
      vim.api.nvim_buf_set_extmark(self.bufnr, highlight_ns, start_line, 0, {
